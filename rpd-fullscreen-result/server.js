...
// Wheel game sound effects — { stop_sound, win_sound }, either can be null
// if admin hasn't uploaded one (the display falls back to a synthesized tone).
app.get('/api/sound-effects', wrap(async (req, res) => {
  const { rows } = await pool.query('SELECT kind, audio FROM sound_effects');
  const out = { stop_sound: null, win_sound: null };
  rows.forEach((r) => { out[r.kind] = r.audio; });
  res.json(out);
}));
...
// ---------- admin: wheel sound effects (cut / hurray) ----------
const VALID_SOUND_KINDS = ['stop_sound', 'win_sound'];

app.put('/api/admin/sound-effects/:kind', requireAdmin, wrap(async (req, res) => {
  const { kind } = req.params;
  if (!VALID_SOUND_KINDS.includes(kind)) return res.status(400).json({ error: 'Unknown sound kind' });
  ...
  await pool.query(
    `INSERT INTO sound_effects (kind, audio) VALUES ($1, $2)
     ON CONFLICT (kind) DO UPDATE SET audio = EXCLUDED.audio, created_at = now()`,
    [kind, audio]
  );
  bumpVersion();
  res.json({ ok: true });
...
app.delete('/api/admin/sound-effects/:kind', requireAdmin, wrap(async (req, res) => {
  const { kind } = req.params;
  if (!VALID_SOUND_KINDS.includes(kind)) return res.status(400).json({ error: 'Unknown sound kind' });
  await pool.query('DELETE FROM sound_effects WHERE kind = $1', [kind]);
  bumpVersion();
  res.json({ ok: true });
}));
