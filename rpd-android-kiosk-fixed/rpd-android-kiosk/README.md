...
`public/index.html` uses a plain
`<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, minimum-scale=1, user-scalable=no">`
and nothing else. That's deliberate — an earlier version had JS that forced
the viewport to `window.screen.width`/`height` to work around kiosk browsers
ignoring the viewport tag, but on the CBOX C1000 (Fully Kiosk Browser) that
override actively broke sizing, because `window.screen` didn't reliably match
the WebView's real CSS pixel width. That's what caused the logo/spin-wheel to
render oversized and cut off at the edges, and promo images to disappear off
the visible area.

If the logo, logo bumper, or spin-wheel elements ever look oversized or
cropped again on a particular box, check the kiosk app's own viewport
settings first, in this order, before touching the page's code:

- **Use Wide Viewport: ON** — without this the kiosk app ignores the page's
  viewport meta tag entirely.
- **View in Desktop Mode: OFF** — forces a fake ~980–1024px viewport.
- **Load in Overview Mode: OFF** — pre-shrinks the page to "fit," fighting
  the page's own exact width.
- **Initial Scale: 100%** and **Set Font Size: 100%** — avoid baked-in zoom.
- **Enable Zoom: OFF** — prevents drift.

Do not reintroduce a JS override that sets the viewport from
`window.screen.width`/`height` — on this hardware that value doesn't match
the real CSS pixel width and recreates the same bug. If a future box needs a
kiosk-specific fix, test it on that box in isolation rather than shipping it
to every device.
